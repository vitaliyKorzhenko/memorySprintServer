...
const getAllLevels = async (req, res) => {
    try {
        const result = await db_1.default.query('SELECT * FROM levels'); // Query to select all levels
...
const getLevelById = async (req, res) => {
    ...
    try {
        const result = await db_1.default.query('SELECT * FROM levels WHERE id = $1', [id]); // Query to select level by ID
...
const findAllLevelProgresses = async (req, res) => {
    try {
        const result = await db_1.default.query('SELECT * FROM level_progress');
...
const findLevelProgressById = async (req, res) => {
    const { id } = req.body;
    try {
        const result = await db_1.default.query('SELECT * FROM level_progress WHERE id = $1', [id]);
        if (result.rows.length > 0) {
...
const createLevelProgress = async (req, res) => {
    ...
    try {
        const result = await db_1.default.query('INSERT INTO level_progress (user_id, level_id, status, attempts, point,  created_at) VALUES ($1, $2, $3, $4, $5, now()) RETURNING *', [user_id, level_id, status, attempts, point]);
        await db_1.default.query('UPDATE users SET points = points + $1 WHERE id = $2', [point, user_id]);
...
const updateLevelProgress = async (req, res) => {
    ...
    try {
        const previousResult = await db_1.default.query('SELECT point, user_id FROM level_progress WHERE id = $1', [id]);
        ...
        const result = await db_1.default.query('UPDATE level_progress SET status = $1, attempts = $2, point = $3, isActive = $4 WHERE id = $5 RETURNING *', [status, attempts, point, isActive, id]);
        await db_1.default.query('UPDATE users SET points = points - $1 + $2 WHERE id = $3', [previousPoint, point, userId]);
        res.status(200).json(result.rows[0]);
    }
    catch (error) {
...
const deleteLevelProgress = async (req, res) => {
    const { id } = req.params;
    try {
        const previousResult = await db_1.default.query('SELECT point, user_id FROM level_progress WHERE id = $1', [id]);
        if (previousResult.rows.length === 0) {
        ...
        const previousPoint = previousResult.rows[0].point;
        const userId = previousResult.rows[0].user_id;
        const result = await db_1.default.query('DELETE FROM level_progress WHERE id = $1 RETURNING *', [id]);
        await db_1.default.query('UPDATE users SET points = points - $1 WHERE id = $2', [previousPoint, userId]);
...
const findUsersByLevelId = async (req, res) => {
    const { level_id } = req.body;
    try {
        const result = await db_1.default.query('SELECT * FROM level_progress WHERE level_id = $1', [level_id]);
        res.status(200).json(result.rows);
    }
    catch (error) {
...
const findProgressByUserId = async (req, res) => {
    const { user_id, ageType } = req.body;
    try {
        const result = await db_1.default.query('SELECT * FROM level_progress lp INNER JOIN levels l ON lp.level_id = l.id WHERE lp.user_id = $1 AND l.ageType = $2', [user_id, ageType]);
        res.status(200).json(result.rows);
    }
    catch (error) {
...
const findUserInfo = async (req, res) => {
    ...
    try {
        const result = await db_1.default.query(`SELECT u.id, u.name, u.email
      FROM users u
      WHERE u.email = $1`, [email]);
        if (result.rows.length > 0) {
            const user = result.rows[0];
            const progress = await db_1.default.query(`SELECT lp.*, l.title
...
const findRandomLevel = async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.default.query(`SELECT l.*
       FROM levels l
       WHERE l.type = $1
         AND l.id NOT IN (
           SELECT level_id
...
const getLevelPackageById = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db_1.default.query('SELECT * FROM level_package WHERE id = $1', [id]);
        if (result.rows.length > 0) {
            res.status(200).json(result.rows[0]);
        }
        else {
